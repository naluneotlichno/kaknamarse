...
func RunningSpentCalories(action int, weight, duration float64) float64 {
    // ваш код здесь
    speed := ((action * lenStep) / mInKm) / duration
    ((runningCaloriesMeanSpeedMultiplier * speed * runningCaloriesMeanSpeedShift) * weight / mInKM * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    speed := ((action * lenStep) / mInKm) / duration * kmhInMsec
    ((walkingCaloriesWeightMultiplier * weight + (Pow(speed, 2) / height) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
}
